...
// ▶️ 算链接（内置逻辑替代远程脚本）
const btnAutoMatch = mkBtn("选链接", async () => {
  showToast("🧮 正在执行算链接...","info");

  const sleep = ms => new Promise(r => setTimeout(r, ms));

  // 读取 mainData
  let input = "";
  try { input = window.mainData || localStorage.getItem("panel_mainData_v1"); } catch(e){}
  if (!input) return showToast("❌ 未检测到 mainData","error");

  const match = input.match(/名称\{([\s\S]*?)\}/);
  if (!match) return showToast("⚠️ mainData 中未找到 名称{} 区块","error");

  const lines = match[1].split(/\n+/).map(l=>l.trim()).filter(Boolean);
  if (!lines.length) return showToast("⚠️ 名称{} 中无有效内容","error");
  ...
  if (!pattern.length) return showToast("❌ 未识别出任何 大/小/超小","error");
  ...
  async function openAndSelect(selectEl, optionText){
    try{
      ...
      await sleep(300);
      const list = document.querySelector('.el-select-dropdown__list, .el-scrollbar__view');
      if(!list) return false;
      const items = Array.from(list.querySelectorAll('.el-select-dropdown__item'));
      const target = items.find(it=>it.innerText.trim().includes(optionText));
      if(target){
        target.scrollIntoView({block:"center"});
        target.click();
        return true;
      }
    }catch(e){console.warn("openAndSelect failed",e);}
    return false;
  }
  ...
  for (let i = 0; i < pattern.length; i++) {
    ...
    if (col5[i]) {
      if (i % 2 === 1) {
        await openAndSelect(col5[i].element, mapCol5[key]);
        console.log(`✅ 第 ${i+1} 行修改 mapCol5 -> ${mapCol5[key]}`);
      } else {
        console.log(`⏩ 第 ${i+1} 行跳过 mapCol5`);
      }
      await sleep(100);
    }
    ...
    // col6 全部执行
    if (col6[i]) {
      await openAndSelect(col6[i].element, mapCol6[key]);
      await sleep(100);
    }

    // col8 全部执行
    if (col8[i]) {
      await fillInputReliable(col8[i].element, mapCol8[key]);
      await sleep(60);
    }
  }

  showToast("✅ 算链接已完成","success");
});
